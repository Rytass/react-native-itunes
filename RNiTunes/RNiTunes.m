...
RCT_EXPORT_METHOD(getTracks:(NSDictionary *)params successCallback:(RCTResponseSenderBlock)successCallback) {
    ...
    NSRange songQueryRange;
    songQueryRange.location = 0;
    songQueryRange.length = MIN(100, songsQuery.items.count);

    NSArray *slicedSongQueryItems = [songsQuery.items subarrayWithRange: songQueryRange];

    for (MPMediaItem *song in slicedSongQueryItems) {
